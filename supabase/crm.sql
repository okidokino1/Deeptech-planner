-- 딥테크 플래너 · CRM 확장 마이그레이션
-- Supabase SQL Editor에 붙여넣어 실행하세요. (setup-all.sql 실행 이후)
...
create index if not exists idx_planning_projects_user on public.planning_projects(user_id);
